...
#include "define.h"
bool corr_des(const point& a, const point& b) // comparison function
{
    return a.r > b.r;
}
...
// 1
			...
			if (i+window_size > bimg2.nr) ke = bimg2.nr-i-1;
			else ke = window_size;
			...
			if (j+window_size > bimg2.nc) le = bimg2.nc-j-1;
			else le = window_size;
...
// 2
			...
			for (int p = 0; p < pts.size(); p++)
			{
				...
				if (i+window_size > bimg2.nr) ke = bimg2.nr-i-1; // max
				else ke = window_size;
				...
				if (j+window_size > bimg2.nc) le = bimg2.nc-j-1; //max
				else le = window_size;
// Compute r
	...
	// Sort by corr value, highest first
	std::sort(pts.begin(), pts.end(), corr_des);
	std::sort(corr.begin(), corr.end(), corr_des);


	// Calculate parameters
	double error = 0, best_error = 9999;
	for (int i = 0; i < pts.size()-2; i++)
	{
		sum = (pts[i+1].x-pts[i+2].x)*(corr[i+0].x-corr[i+1].x) + (pts[i+1].y-pts[i+0].y)*(pts[i+1].x-pts[i+2].x) + (pts[i+1].y-pts[i+2].y)*(pts[i+0].x-pts[i+1].x);
		b = (corr[i+1].x-corr[i+2].x)*(pts[i+0].x-pts[i+1].x) / sum;
		a = ((corr[i+0].x-corr[i+1].x) - b*(pts[i+0].y-pts[i+1].y)) / (pts[i+0].x-pts[i+1].x);
		c = corr[i+2].x - a*pts[i+2].x - b*pts[i+2].y;

		sum = (pts[i+1].x-pts[i+2].x)*(corr[i+0].y-corr[i+1].y) + (pts[i+1].y-pts[i+0].y)*(pts[i+1].x-pts[i+2].x) + (pts[i+1].y-pts[i+2].y)*(pts[i+0].x-pts[i+1].x);
		e = (corr[i+1].y-corr[i+2].y)*(pts[i+0].x-pts[i+1].x) / sum;
		d = ((corr[i+0].y-corr[i+1].y) - b*(pts[i+0].y-pts[i+1].y)) / (pts[i+0].x-pts[i+1].x);
		f = corr[i+2].y - a*pts[i+2].x - b*pts[i+2].y;
